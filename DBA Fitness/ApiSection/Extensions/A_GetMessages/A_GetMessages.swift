...
extension UIViewController{
    ...
    func getMessages(parameters:[String :Any] ){
        if Connectivity.isConnectedToInternet {
            NVActivityIndicator.managerHandler.showIndicator()
            
            apimethod.commonMethod(url: ApiURLs.get_all_messages, parameters: parameters, method: "GET") { (_ dict, _ success: Bool , _ error: Error?,_ response: Any, _ responseData) in
                ...
                if success, let result = response as? HTTPURLResponse{
                    ...
                    if(result.statusCode==404||result.statusCode==400 || result.statusCode==401){
                        ...
                        getMessageInfo = try? JSONDecoder().decode(M_GetMessages.self, from: responseData)
