...
extension UIViewController{
    ...
    func addSuppliment(parameters:[String :Any] ){
        if Connectivity.isConnectedToInternet {
            NVActivityIndicator.managerHandler.showIndicator()
            
            apimethod.commonMethod(url: ApiURLs.add_suppliment, parameters: parameters, method: "POST") { (_ dict, _ success: Bool , _ error: Error?,_ response: Any, _ responseData) in
                ...
                if success, let result = response as? HTTPURLResponse{
                    ...
                    if(result.statusCode==404||result.statusCode==400 || result.statusCode==401){
                        ...
                        let refreshAlert = UIAlertController(title: "DBA Fitness", message: "Supplement updated", preferredStyle: UIAlertController.Style.alert)
