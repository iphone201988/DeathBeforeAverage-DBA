...
extension UIViewController{
    ...
    func forgotPassword(parameters:[String :Any] ){
        if Connectivity.isConnectedToInternet {
            ...
            apimethod.commonMethod(url: ApiURLs.forgot_password, parameters: parameters, method: "POST") { (_ dict, _ success: Bool , _ error: Error?,_ response: Any, _ responseData) in
                ...
                if success, let result = response as? HTTPURLResponse{
                    ...
                    if(result.statusCode==404||result.statusCode==400 || result.statusCode==401){
                        ...
                        let refreshAlert = UIAlertController(title: "DBA Fitness", message: "\(dict?["message"] ?? "Something went wrong, please retry., please retry.")", preferredStyle: UIAlertController.Style.alert)
