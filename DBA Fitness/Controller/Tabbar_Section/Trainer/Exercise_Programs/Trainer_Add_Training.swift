...
class Trainer_Add_Training: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        selectedWorkoutDayTV.delegate = self
    ...
    @IBAction func save(_ sender: UIButton) {
        /*
         if let count = trainerProgram?.trainings?.count{
         if count > 0{
         for dict in (trainerProgram?.trainings) {
         trainingDict["description"] = dict.trainingDescription
         trainingDict["name"] = dict.name
         trainingDict["days"] = dict.days
         if let exerciseCount = dict.exercises?.count {
         if exerciseCount > 0{
         for exerciseDict in dict.exercises {
         let id = exerciseDict.id
         exerciseID.append(id)
         }
         trainingDict["exercises"] = exerciseID
         }else{
         trainingDict["exercises"] = exerciseID
         }
         }
         trainingArray.append(trainingDict)
         exerciseID.removeAll()
         }
         }
         }*/
        
        let desc = about_TV.text.trimmed()
        let name = trainingNameTV.text.trimmed()
        
        if name == "" || name == "Ex: Arms & Abs" || desc == "" || desc == "Write About Workout" || stringFormatSelectedDaysID == ""{
            
            if name == "" || name == "Ex: Arms & Abs"{
                UniversalMethod.universalManager.alertMessage("Training name cannot be empty", self.navigationController)
            }else if (desc == "" || desc == "Write About Workout"){
            ...
            addProgramTraining(program_id:particularPrograms?.id ?? "0", training_name:name , training_description:desc, training_video:videoURL, training_day:stringFormatSelectedDaysID, apiUrl:ApiURLs.add_training, training_id:"", thumbnil: thumbnil)
        }
    }
...
extension Trainer_Add_Training: UIPickerViewDelegate, UIPickerViewDataSource{
...
extension Trainer_Add_Training: UITextViewDelegate{
    ...
    func textViewDidBeginEditing(_ textView: UITextView) {
        ...
        } else if textView == selectedWorkoutDayTV{
            if textView.textColor == UIColor.lightGray{
                textView.text = ""
                textView.textColor = UIColor.white
            }
    ...
    func textViewDidEndEditing(_ textView: UITextView) {
        ...
        if textView == about_TV{
            ...
            if textView.text == ""  {
                textView.text = "Choose workout days"
                textView.textColor = UIColor.lightGray
            }
        }
    }
...
extension Trainer_Add_Training: UIImagePickerControllerDelegate, UINavigationControllerDelegate{
...
extension Trainer_Add_Training: UITableViewDelegate, UITableViewDataSource{
