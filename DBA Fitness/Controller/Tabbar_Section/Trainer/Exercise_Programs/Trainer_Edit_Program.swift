...
class Trainer_Edit_Program: UIViewController,UITextFieldDelegate {
    ...
    @IBOutlet weak var trainingGoalTV: UITextView!
    ...
    override func viewDidLoad() {
      ...
      //  program_Name.delegate = self
        ...
        trainingGoalTV.textContainerInset = UIEdgeInsets.zero;
        trainingGoalTV.textContainer.lineFragmentPadding = 0;
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        trainingGoalTV.text = particularPrograms?.goals
        dayOfWeek.text = particularPrograms?.daysPerWeek
        price.text = particularPrograms?.price
        gender.text = particularPrograms?.sex
        category.text = particularPrograms?.category
        training_Level.text = particularPrograms?.levelOfTraining
        let programName = particularPrograms?.programName ?? ""
        let programDescription = particularPrograms?.programDescription ?? ""
        if programName.isEmpty {
            programNameTV.text = "Ex: Off Season Strength & Conditioning"
            programNameTV.textColor = UIColor.lightGray
        } else {
            programNameTV.text = programName
            programNameTV.textColor = UIColor.white
        }
        ...
        if programDescription.isEmpty {
            ...
            about_TV.text = programDescription
            about_TV.textColor = UIColor.white
        }
    }
    ...
    @IBAction func next(_ sender: UIButton) {
        ...
        if programName == "" || programName == "Ex: Off Season Strength & Conditioning" || aboutTV == "" || aboutTV == "Write a Program Description" || training_Level.text == "" || trainingGoalTV.text == "" || price.text == "" || gender.text == "" || dayOfWeek.text == "" || category.text == ""{
            UniversalMethod.universalManager.alertMessage("All fields are required", self.navigationController)
        }else{
            Constants.isEditProgram = "1"
            
            let replaced = price.text?.replacingOccurrences(of: "$", with: "")
            
            addProgram(parameters:["program_id":particularPrograms?.id ?? "0","program_name":programName ?? "","program_description":aboutTV ?? "","level_of_training":training_Level.text ?? "","goals":trainingGoalTV.text ?? "","price":replaced ?? "","category":category.text ?? "","sex":gender.text ?? "","days_per_week":dayOfWeek.text ?? "",] )
        }
    }
...
extension Trainer_Edit_Program: UITextViewDelegate{
...
extension Trainer_Edit_Program: UIPickerViewDelegate, UIPickerViewDataSource{
