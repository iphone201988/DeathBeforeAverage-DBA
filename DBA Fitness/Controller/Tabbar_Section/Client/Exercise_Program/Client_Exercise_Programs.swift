...
class Client_Exercise_Programs: UIViewController {
    ...
    func showCataloguePopup(_ catalogueName:String? = nil, _ catalogueID:String? = nil){
        ...
        let alertController = UIAlertController(title: titleValue, message: "", preferredStyle: UIAlertController.Style.alert)
        ...
        let saveAction = UIAlertAction(title: alertTitle, style: UIAlertAction.Style.default, handler: { alert -> Void in
            ...
            if catalogueName != ""{
                ...
                if catalogueName?.count ?? 0 > 50{
                    ...
                    if catalogueID == nil{
                        self.callCreateExerciseCatalogue(parameters: ["title":alertController.textFields?[0].text ?? ""], true)
                    }else{
                        self.callCreateExerciseCatalogue(parameters: ["title":alertController.textFields?[0].text ?? "", "folder_id":catalogueID ?? ""], false)
                    }
                }
            }else{
...
extension Client_Exercise_Programs: UITableViewDelegate, UITableViewDataSource{
        ...
        if isExercise_Section == true{
            ...
            if let cell = tableView.dequeueReusableCell(withIdentifier: "Client_Progress", for: indexPath) as? Client_Progress {
                ...
                if isTrainer == "Trainer"{
                    ...
                    cell.rightButtons = [deleteButton,editButton, sendButton]
                }else{
                    let deleteButton = RowActionButton(title: "", icon: #imageLiteral(resourceName: "delete_icon"), backgroundColor: #colorLiteral(red: 1, green: 0.231372549, blue: 0.2196078431, alpha: 1), padding: 15) { [weak self, indexPath] _ -> Bool in
                        self?.removeExercise(at: indexPath.row)
                        return true
    ...
    private func removeProgram(at index: Int) {
        ...
        addProgram(parameters:["program_id":particularPrograms?.id ?? "0"] )
    ...
    private func removeExercise(at index: Int) {
        ...
        let particularCatalogueInfo = exerciseCatalogueInfos?.data?[index]
        callDeleteExerciseCatalogue(parameters:["exercise_id":"\(particularCatalogueInfo?.id ?? "")"])
        //deleteExercise(parameters:["excercise_id" :exerciseInfos?.data?[index].id ?? "0"] )
    }
    
    private func editGoal(at index: Int) {
        let particularCatalogueInfo = exerciseCatalogueInfos?.data?[index]
        showCataloguePopup(particularCatalogueInfo?.title, particularCatalogueInfo?.id)
    }
...
extension Client_Exercise_Programs: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
...
extension Client_Exercise_Programs: MFMailComposeViewControllerDelegate{
