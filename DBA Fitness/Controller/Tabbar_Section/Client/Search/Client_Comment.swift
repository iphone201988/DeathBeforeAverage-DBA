...
class Client_Comment: UIViewController {
    ...
    var chatMessagesArray = [M_GetTrainerCommentsData]()
    ...
    override func viewDidLoad() {
        ...
        if let photo = DataSaver.dataSaverManager.fetchData(key: "profilePic") as? String{//searchedTrainerInfo?.image{
            if photo != ""{
    ...
    @objc func updatedPost(_ notify:NSNotification) {
        ...
        if isGetted == true{
            ...
            let dataStatus = trainerCommentsInfo?.status
            if (dataStatus == 200 && trainerCommentsInfo?.data != nil){
                
                if let count = trainerCommentsInfo?.data?.count{
                    let olderMsgs : [M_GetTrainerCommentsData] = trainerCommentsInfo?.data ?? [] //(trainerCommentsInfo?.data?.reversed())
    ...
    override func viewWillAppear(_ animated: Bool) {
        Constants.isRatingTrainers = "0"
        trainerCommentsRating(parameters:["trainer_id": userID] )
    }
    ...
    @IBAction func sendMessage(_ sender: UIButton) {
        ...
        if NVActivityIndicator.managerHandler.isIndicatorVisible(){
            ...
            if messageText == ""{
                ...
                isCommented = true
                trainerCommentsRating(parameters:["trainer_id": userID, "comment":messageText] )
                //self.scrollToBottom()
...
extension Client_Comment: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "Comment_Cell", for: indexPath) as? Comment_Cell {
            ...
            // upload/user_img/d52f845091a567a69b6656ab140d8eff.jpg
...
extension Client_Comment: UITextViewDelegate{
